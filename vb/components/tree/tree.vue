  ...
  <ul :class="treeCls">
    <li v-for="(item, index) in data" :class="{[`${prefixCls}-treenode-disabled`]: item.disabled,[dropOverCls]: dragOverIndex === index,'filter-node': item.filter}" @dragover="dragover" @drop="drop(index,$event)" @dragenter="dragenter(index,$event)" @dragleave="dragleave(index,$event)" ref="node">
      ...
      <transition name="slide-up">
        <tree v-if="!item.isLeaf" :prefix-cls="prefixCls" :data="item.children" :clue="`${clue}-${formatClue(index)}`" :multiple="multiple" :checkable="checkable" :class="`${prefixCls}-child-tree-open`" v-show="item.expanded" :draggable="draggable" :async="async"></tree>
      </transition>
    </li>
  </ul>
</template>
...
export default {
    ...
    mounted() {
        ...
        this.$on('nodeSelected', (params) => {
            if (this.clue !== this.clueDefault) return this.dispatch('Tree', 'nodeSelected', params);
        ...
        this.$on('childChecked', (params) => {
            ...
            for (const [i, item] of this.data.entries()) {
                if (`${this.clue}-${this.formatClue(i)}` === params.clue) {
        ...
        this.$on('dragdrop', (sourceClue, targetClue, dropPosition) => {
            if (this.clue !== this.clueDefault) return this.dispatch('Tree', 'dragdrop', [sourceClue, targetClue, dropPosition]);
            // 直接父级是否是同一个
    ...
    methods: {
        ...
        dragstart(index, ev) {
            ...
            ev.dataTransfer.setData('dragClue', `${this.clue}-${this.formatClue(index)}`);
        },
        dragover(ev) {
            ev.preventDefault();
            ev.stopPropagation();
        ...
        drop(index, ev) {
            ...
            const selfClue = `${this.clue}-${this.formatClue(index)}`;
        ...
        setKey() {
            for (let i = 0; i < this.data.length; i++) {
                this.data[i].clue = `${this.clue}-${this.formatClue(i)}`;
            }
        },
        preHandle() {
            for (const [i, item] of this.data.entries()) {
                ...
                if (item.checked && !item.childrenCheckedStatus) {
                    this.$set(item, 'childrenCheckedStatus', 2);
                    this.broadcast('Tree', 'parentChecked', { status: true, clue: `${this.clue}-${this.formatClue(i)}` });
                } else {
        ...
        setCheck(disabled, index) {
            ...
            this.broadcast('Tree', 'parentChecked', { status: checked, clue: `${this.clue}-${this.formatClue(index)}` });
        },
        getNodes(data, opt) {
            data = data || this.data;
            let res = [];
        ...
        addNode(path, data) {
            if (path === this.clueDefault) return this.data.push(data);
